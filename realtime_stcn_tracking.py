...
class RealtimeSTCNTracker(Node):
    def __init__(self):
        ...
        # QoS profile for sensors (best effort, keep last)
        ...
        self.sub_info01 = self.create_subscription(
            CameraInfo, '/ob_camera_01/depth/camera_info', self.callback_info01, sensor_qos
        )
        self.sub_info02 = self.create_subscription(
            CameraInfo, '/ob_camera_02/depth/camera_info', self.callback_info02, sensor_qos
        )

        # Subscribers - Color images
        self.sub_cam01 = self.create_subscription(
            Image, '/ob_camera_01/color/image_raw', self.callback_cam01_color, sensor_qos
        )
        self.sub_cam02 = self.create_subscription(
            Image, '/ob_camera_02/color/image_raw', self.callback_cam02_color, sensor_qos
        )

        # Subscribers - Depth images
        self.sub_cam01_depth = self.create_subscription(
            Image, '/ob_camera_01/depth/image_raw', self.callback_cam01_depth, sensor_qos
        )
        self.sub_cam02_depth = self.create_subscription(
            Image, '/ob_camera_02/depth/image_raw', self.callback_cam02_depth, sensor_qos
        )

        # Publisher - Combined human pointcloud in base_link frame
        self.pub_human_pc = self.create_publisher(
            PointCloud2, '/human_pointcloud', 10
        )

        # Publishers - Masks (for debugging)
        self.pub_mask01 = self.create_publisher(
            Image, '/ob_camera_01/human_mask', 10
        )
        self.pub_mask02 = self.create_publisher(
            Image, '/ob_camera_02/human_mask', 10
        )
    ...
    def update_memory_bank_with_increment(self, frame_tensor, increment_mask, orig_size):
        ...
        try:
            ...
            if increment_pixels < 100:  # Too small to be meaningful
                self.get_logger().warn("Increment mask too small (%d pixels), skipping" % increment_pixels)
                return
    ...
    def grow_mask_with_depth_clustering(self, rgb_mask, depth_image):
        ...
        if seed_pixels < self.min_cluster_size:
            self.get_logger().warn("Seed mask too small (%d pixels), skipping depth clustering" % seed_pixels)
            return rgb_mask
        ...
        if len(valid_depths) < 100:
            self.get_logger().warn("Not enough valid depth values in seed region, skipping clustering")
            return rgb_mask
        ...
        if len(seed_labels) == 0:
            self.get_logger().warn("No depth cluster overlaps with seed mask")
            return rgb_mask
    ...
    def transform_and_combine_pointclouds(self, points01, points02, timestamp):
        ...
        Args:
            ...
            timestamp: rclpy Time timestamp for TF lookup
        ...
        if points01 is not None and len(points01) > 0:
            try:
                ...
                transform = self.tf_buffer.lookup_transform(
                    'base_link', 'ob_camera_01_link', timestamp, Duration(seconds=0.1)
                )
                ...
                self.get_logger().warn("Failed to lookup transform for camera 01: %s" % str(e))
        ...
        if points02 is not None and len(points02) > 0:
            try:
                ...
                from rclpy.duration import Duration
                transform = self.tf_buffer.lookup_transform(
                    'base_link', 'ob_camera_02_link', timestamp, Duration(seconds=0.1)
                )
                ...
                self.get_logger().warn("Failed to lookup transform for camera 02: %s" % str(e))
    ...
    def remove_ground_plane(self, points):
        ...
        try:
            ...
            if len(filtered_points) < self.min_points_above_ground:
                self.get_logger().warn("[Ground Removal] Too few points remaining (%d), returning original" % len(filtered_points))
                return points
    ...
    def process_frame(self):
        ...
        if not self.tracking_initialized:
            ...
            if is_lost:
                ...
                self.get_logger().warn("Poor tracking detected [%s] - Quality: %.2f - Frame %d (%d/%d)" %
                ...
                if self.poor_tracking_count >= self.tracking_lost_threshold:
                    ...
                    if new_mask is not None:
                        ...
                        self.get_logger().warn("YOLO re-detection failed, using fallback mask")
        ...
        if mask is not None:
            ...
            if depth01 is not None and depth02 is not None:
                ...
                try:
                    ...
                    mask_msg01.header.stamp = self.get_clock().now().to_msg()
                    ...
                    mask_msg02.header.stamp = self.get_clock().now().to_msg()
